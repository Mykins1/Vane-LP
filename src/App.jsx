...
const VaneIcon = () => (
  <svg width="26" height="26" viewBox="0 0 28 28" fill="none" style={{flexShrink:0}}>
    ...
    <path d="M14 18 L14 24" stroke="#1D4ED8" strokeWidth="2" strokeLinecap="round"/>
  </svg>
);
const StripeIcon = () => (
  <svg width="18" height="18" viewBox="0 0 20 20" fill="none" style={{flexShrink:0}}>
    <rect width="20" height="20" rx="4" fill="#635BFF"/>
    <path d="M10 6c-1.5 0-2.5.6-2.5 1.6 0 1.8 3.5 1.7 3.5 3.2 0 .7-.6 1.1-1.6 1.1-.9 0-1.7-.3-2.4-.7v1.7c.7.3 1.5.5 2.4.5 1.6 0 2.7-.7 2.7-1.9 0-1.9-3.5-1.8-3.5-3.2 0-.6.5-1 1.4-1 .8 0 1.5.2 2.1.5V6.4C11.4 6.1 10.7 6 10 6z" fill="white"/>
  </svg>
);
const GmailIcon = () => (
  <svg width="18" height="18" viewBox="0 0 20 20" fill="none" style={{flexShrink:0}}>
    ...
    <rect x="4" y="6" width="12" height="9" rx="1" stroke="white" strokeWidth="1.5" fill="none"/>
  </svg>
);
const NotionIcon = () => (
  <svg width="18" height="18" viewBox="0 0 20 20" fill="none" style={{flexShrink:0}}>
    <rect width="20" height="20" rx="4" fill="#191919"/>
    <text x="5" y="15" fontSize="12" fill="white" fontWeight="bold">N</text>
  </svg>
);
const SlackIcon = () => (
  <svg width="18" height="18" viewBox="0 0 20 20" fill="none" style={{flexShrink:0}}>
...
function FeedMockup() {
  ...
  useEffect(() => {
    FEED_CARDS.forEach((c,i) => setTimeout(() => setShown(p=>[...p,c.id]), 350 + i*200));
  }, []);
  return (
    <div style={{background:"#fff",border:"1px solid #E5E7EB",borderRadius:14,
      boxShadow:"0 16px 56px -8px rgba(0,0,0,0.11),0 4px 16px -2px rgba(0,0,0,0.05)",
      overflow:"hidden",maxWidth:560,width:"100%",margin:"0 auto",boxSizing:"border-box"}}>
      {/* chrome */}
      <div style={{background:"#F9FAFB",borderBottom:"1px solid #E5E7EB",padding:"10px 14px",
        ...
        <div style={{flex:1,textAlign:"center",fontSize:11,color:"#9CA3AF",fontFamily:"monospace",
          overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>vane.io/feed</div>
      </div>
      {/* feed header */}
      <div style={{padding:"12px 14px 6px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        <span style={{fontWeight:700,fontSize:13,color:"#1A1A18"}}>Your feed</span>
        <span style={{fontSize:11,color:"#9CA3AF"}}>4 new · updated now</span>
      </div>
      {/* cards */}
      <div style={{padding:"2px 10px 12px"}}>
        {FEED_CARDS.map(card=>(
          <div key={card.id} style={{
            ...
            transform: shown.includes(card.id)?"translateY(0)":"translateY(12px)",
            transition:"opacity 0.38s ease,transform 0.38s ease",
            background:"#fff",
            border:"1px solid #E5E7EB",
            borderLeft: card.urgent?"3px solid #E24B4A":"1px solid #E5E7EB",
            borderRadius:9,padding:"10px 11px",marginBottom:7,
            boxSizing:"border-box",minWidth:0,
          }}>
            {/* top row */}
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",
              marginBottom:3,gap:6,minWidth:0}}>
              <div style={{display:"flex",alignItems:"center",gap:6,minWidth:0,flexShrink:1,overflow:"hidden"}}>
                <span style={{width:7,height:7,borderRadius:"50%",background:card.dot,
                  display:"inline-block",flexShrink:0}}/>
                <span style={{fontSize:9,fontWeight:700,textTransform:"uppercase",
                  letterSpacing:"0.05em",color:"#6B7280",flexShrink:0}}>{card.source}</span>
                {card.urgent&&(
                  <span style={{background:"#FEF2F2",color:"#E24B4A",fontSize:9,fontWeight:700,
                    padding:"1px 6px",borderRadius:20,border:"1px solid #FECACA",
                    flexShrink:0,whiteSpace:"nowrap"}}>Urgent</span>
                )}
              </div>
              <span style={{fontSize:10,color:"#9CA3AF",whiteSpace:"nowrap",flexShrink:0}}>{card.time}</span>
            </div>
            {/* title + subtitle — clipped, never overflow */}
            <div style={{fontSize:12,fontWeight:700,color:"#1A1A18",marginBottom:2,
              overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{card.title}</div>
            <div style={{fontSize:11,color:"#6B7280",marginBottom:8,
              overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{card.subtitle}</div>
            {/* actions */}
            <div style={{display:"flex",gap:5,flexWrap:"wrap"}}>
              {card.actions.map(a=>(
                <button key={a} style={{fontSize:10,fontWeight:600,padding:"3px 9px",
                  borderRadius:5,border:"1px solid #E5E7EB",background:"#F9FAFB",
                  color:"#374151",cursor:"pointer",fontFamily:"inherit",whiteSpace:"nowrap"}}>{a}</button>
              ))}
            </div>
          </div>
        ))}
...
function Section({id, bg, children}) {
  ...
  return (
    <section id={id} ref={ref} style={{
      ...
      <div style={{maxWidth:1080,margin:"0 auto",padding:"0 20px",boxSizing:"border-box",width:"100%"}}>
        {children}
      </div>
    </section>
  );
...
const SectionLabel = ({children}) => (
  <p style={{fontSize:10,fontWeight:700,letterSpacing:"0.13em",textTransform:"uppercase",
    color:"#6B7280",marginBottom:14}}>{children}</p>
);

const SectionH2 = ({children, mb=40}) => (
  <h2 style={{fontSize:"clamp(24px,5vw,38px)",fontWeight:800,lineHeight:1.12,
    color:"#1A1A18",marginBottom:mb}}>{children}</h2>
);

/* Problem section*/
const PROBLEMS = [
  {icon:<ToolIcon/>, title:"Tool overload",
  ...
  {icon:<SwitchIcon/>, title:"Constant switching",
  ...
  {icon:<SlipIcon/>, title:"Things fall through",
...
const LOGOS = [
  ...
  {name:"Notion",el:<NotionIcon/>},{name:"Slack",el:<SlackIcon/>},
];
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(min(100%,240px),1fr))",gap:28,marginBottom:44}}>
        {STEPS.map(s=>(
          <div key={s.n} style={{boxSizing:"border-box"}}>
            <div style={{fontSize:42,fontWeight:900,color:"#EEEEED",lineHeight:1,marginBottom:10,
              fontVariantNumeric:"tabular-nums"}}>{s.n}</div>
            <div style={{fontWeight:700,fontSize:15,color:"#1A1A18",marginBottom:8}}>{s.title}</div>
            <p style={{fontSize:14,color:"#6B7280",lineHeight:1.65,margin:0}}>{s.text}</p>
          </div>
        ))}
      </div>
      {/* integration chips */}
      <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
        {LOGOS.map(l=>(
          <div key={l.name} style={{display:"flex",alignItems:"center",gap:7,
            ...
            {l.el}
            <span style={{fontSize:13,fontWeight:600,color:"#374151"}}>{l.name}</span>
          </div>
        ))}
        <span style={{fontSize:13,color:"#9CA3AF"}}>+ more coming soon</span>
      </div>
    </Section>
  );
}
  ...
  return (
    <Section id="waitlist" bg="#F5F5F3">
      <div style={{maxWidth:560,margin:"0 auto",textAlign:"center",boxSizing:"border-box",width:"100%"}}>
        ...
        {done ? (
          <div style={{background:"#fff",border:"1px solid #D1FAE5",borderRadius:12,
            padding:"28px 20px",maxWidth:360,margin:"0 auto",boxSizing:"border-box"}}>
            <div style={{fontSize:20,color:"#22C55E",marginBottom:8}}>✓</div>
            <div style={{fontWeight:700,fontSize:16,color:"#1A1A18",marginBottom:6}}>You're on the list.</div>
            <div style={{fontSize:14,color:"#6B7280"}}>We'll be in touch when your spot opens up.</div>
          </div>
        ) : (
          <>
            ...
            <label style={{display:"flex",alignItems:"flex-start",justifyContent:"center",
              gap:10,cursor:"pointer",marginTop:16,maxWidth:480,margin:"16px auto 0",
              boxSizing:"border-box"}}
              ...
              <span style={{fontSize:13,color:"#6B7280",textAlign:"left",lineHeight:1.5}}>
